...
const forecast = (latitude, longitude, callback) => {
    ...
    request({url, json: true}, (error, {body}) => {// json:true-> Parsea automaticamente la respuesta //el segundo argumento de request es nuestro callback
        if (error) {
            ...
            callback (undefined, body.current.weather_descriptions[0] + '. It is currently '+ body.current.temperature + ' degrees out. It feels like '+ body.current.feelslike +' degress out. The humidity is '+ body.current.humidity +'% and wind speed is '+ body.current.wind_speed+ 'km/h.')
        }
    })
